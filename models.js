module.exports = {
	...
	selectArticles: (author, title, topic, sortBy, order) =>
		database(
        ...
        ` +
									((processedWhereClause = [
										...
										.reduce(
											(whereClause, filter) =>
												`${whereClause} ${
													filter ? `${whereClause.length > 7 ? 'AND' : ''} ${filter}` : ''
												}`,
											'WHERE'
										)
										.trim()) === 'WHERE'
    ...
    GROUP BY articles.article_id
        ORDER BY ${
					sortBy === 'comment_count'
						...
						: `articles.${sortBy || 'created_at'} ${order || 'DESC'}`
				}`
							),
    ...
    GROUP BY articles.article_id;`,
			...
			article ? article : Promise.reject({ status: 404, message: 'Article Not Found' })
		),
    ...
    ORDER BY created_at DESC;`,
			...
			comments.length ? comments : Promise.reject({ status: 404, message: 'Article Not Found' })
		),

	insertCommentToArticle: ({ article_id, body: { username, body } }) =>
		database(
			`    
    ...
    RETURNING *`,
			...
			comment.length ? comment : Promise.reject({ status: 404, message: 'Comment Not Found' })
		),

	readEndpointJSON: () => require('fs/promises').readFile('./ENDpoints.json', 'utf8'),

	_formattedConnection: (database = (psql, ...replacements) =>
		require('./db/connection.js').query(require('pg-format')(psql, ...replacements))),
}
