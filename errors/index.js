module.exports = {
  customError: ({ status, message }, response, next) => {
  ...
  psqlError: (error, response) => {
    ...
    if (['ri_ReportViolation', 'pg_strtoint32', 'ExecConstraints'].some(psqlRoutine => psqlRoutine === routine)) return response.status(400).send('Bad Request')
  ...
  serverError: response => response.status(500).send('There appears to be an internal server error.'),
}